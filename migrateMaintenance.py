...
def parse_args():
    ...
    parser.add_argument('--ignore-apps', dest='ignore', type=str, help='Apps not to migrate')
...
def migrate_tasks(marathon_endpoint, instances, tasks, hosts, ignores):
    """
    Migrate tasks from the hosts going to go for maintenance
    """
    for appId in tasks.iterkeys():
        if all(ignore not in appId for ignore in ignores):
    ...
    marathon_apps_url = marathon_endpoint + apps_endpoint

    for appId in tasks.iterkeys():
        if all(ignore not in appId for ignore in ignores):
            check_deploy_status(marathon_apps_url, appId)


def main():
    args = parse_args()
    maintenance_hosts = args.hosts.split(',')
    ignore_apps = args.ignore.split(',')
    ...
    # Tasks migration
    migrate_tasks(args.url, running_instances, running_tasks, maintenance_hosts, ignore_apps)
